...
- **Reach it over IPv6, never `100.107.144.35`**: the apartment LAN also uses `100.64.0.0/10`, so the Mac routes that IPv4 out wifi and every port times out while `tailscale ping` still pongs — use `ssh homelab@fd7a:115c:a1e0::bc01:90d2` (aliases `hlstatus`/`hlconnect` at `~/.zshrc:19,24` are zsh-only, and the Bash sandbox has no `utun` route).
